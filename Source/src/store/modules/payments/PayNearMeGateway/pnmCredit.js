...
const getters = {
  isPNMCreditAvailable: () => state.isPNMCreditAvailable,
...
const actions = {
  // part 1: flow to determine if we should display an instument (flag isPNMCreditAvailable: true or false)
  // instrument will be displayed if allowed and active
  isPNMCreditAvailable: async (context) => {
    const isAllowed = await context.dispatch('isPNMCreditAllowed')
    if (!isAllowed) {
      state.isPNMCreditAvailable = false
    } else {
      const isActive = await context.dispatch('isPNMCreditActive')
      if (isActive) {
        state.isPNMCreditAvailable = true
      } else {
        context.dispatch('registerInstrument')
      }
    }
  },
  isPNMCreditAllowed (context) {
    const allowedPaymentTypes = context.rootGetters['payments/allowedPaymentTypes']
    return allowedPaymentTypes && allowedPaymentTypes.find(type => type.IDMMSIType === 'PNMCredit')
  },
  async isPNMCreditActive (context) {
  ...
  getActiveInstrument: async (context, type) => {
    try {
      ...
      return instuments.find(type => type.IDMMSIType === 'PNMCredit')
    } catch (exception) {
      console.log(`Error when trying to fetch a PNMCredit instrument of type: (${type})`)
    }
  },
  registerInstrument: async (context) => {
    try {
      await lib.rpcService.callBroker('AccountService', 'RegisterRedirectBasedWallet', {'IDMMSIType': 'PNMCredit'})
      await context.dispatch('payments/initializeActiveInstruments', null, { root: true })
      state.isPNMCreditAvailable = await context.dispatch('isPNMCreditActive')
    } catch (exception) {
      console.log(`Error when trying to register a PNMCredit instrument`)
    }
  },
  // part 1 ends here
