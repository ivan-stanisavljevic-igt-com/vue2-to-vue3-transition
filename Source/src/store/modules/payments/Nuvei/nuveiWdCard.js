...
const paymentIDMMSIType = 'NuveiWdCard'
...
const actions = {
  ...
  start: async (context, payload) => {
    ...
    const { result } = await context.dispatch('getIDMMSITXRequest', payload)
    if (result) {
  ...
  async getIDMMSITXRequest (context, { amount, instrument }) {
    try {
      const IDMMSITXRequest = await lib.rpcService.callBroker('AccountService', 'BeginPaymentTransactionWithoutPassword', {
        IDMMSIInstruction: instrument ? instrument.IDMMSIInstruction : state.withdrawalInstrument.IDMMSIInstruction,
