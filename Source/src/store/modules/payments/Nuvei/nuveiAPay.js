...
const paymentIDMMSIType = 'NuveiAPay'
...
const state = {
  ...
  successMsg: null,
  infoMsg: null,
  applePayProxyScriptIsLoaded: false
}
const getters = {
...
const actions = {
  ...
  async openIFrame (context, url) {
    let decoratedUrl = `${url}`
    let applePayProxyScriptURL = config && config.app.autoconf.NUVEI_APPLEPAY_PROXY_URL
    if (!applePayProxyScriptURL) {
      console.error(`${paymentIDMMSIType} | missing configuration in env-config.js | Parameter name: NUVEI_APPLEPAY_PROXY_URL `)
      return
    }
    if (!state.applePayProxyScriptIsLoaded) {
      await paymentHelpers.loadScriptAsync(applePayProxyScriptURL)
      state.applePayProxyScriptIsLoaded = true
    }
    try {
      ...
      console.log(`${paymentIDMMSIType}.openIFrame | ERROR | ${error && error.message}`)
    }
  },
