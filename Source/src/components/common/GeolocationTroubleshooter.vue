      ...
      <template v-if="checkType()">
        <div class="header" :class="{'simple':(message.errorCode === 612 || errorMsgLocalStorage === '612')}">
          <v-layout align-center>
            <v-flex>
              <h1 class="">{{ $t('Geolocation.ErrorMessage.Header') }}</h1>
          ...
          <v-layout align-center>
            <v-flex>
              <div class="troubleshooter-container">
                <v-card @scroll="handleScroll()" class="troubleshooter-msg" v-if="message.errorCode && message.errorCode !== 612">
                ...
                <div v-if="message.errorCode !== 612 && !hasScrolledToBottom && geocomplyTroubleshooter.length > 1" class="bottom-fade"></div>
              </div>
              ...
              <i v-if="message.errorCode === 612" class="space-btw"></i>
        ...
        <div class="geo-content">
            <div class="buttons">
              ...
              <v-btn v-if="retry || message.errorCode === 612" block @click.stop="geolocateAgain()" class="try-again-btn">
                <span class="gc-link">Try Again</span>
              </v-btn>
              <v-btn v-if="message.errorCode !== 612" block @click.stop="closeDialog()" class="okay-btn">
                <span class="gc-link">{{ $t('Geolocation.ErrorMessage.OK') }}</span>
              </v-btn>
              <v-btn v-if="message.errorCode === 612" block @click.stop="closeDialog()" class="maybe-later-btn">
                <span class="gc-link">Maybe later</span>
              </v-btn>
            </div>
          </div>
      </template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      pluginDownload () {
        console.log('Downloading geolocation plugin...', 'https://' + this.walletParams.installerURL + '/installer/url?id=' + this.walletParams.installerKey + '&os=' + this.navigatorID + '&version=' + this.walletParams.geocomplyPluginVersion + '&user_id=' + this.message.userID)
