    ...
    <v-layout  class="page-layout  add-funds">
      <v-flex class="main-column" ref="maincolumn" >
        <v-form
          ...
          v-model="valid"
          lazy-validation>
          <h1 v-if="!showSightlineImg">{{ $t('Transaction.Sightline.Funding.Title') }}</h1>
          <img v-if="showSightlineImg" class="PlayPlusImg" :src="mediaServer('/static/brand-img/' + brandKey + '/sightline.png')" alt="Sightline"/>
          <p class="generic-icon-note"><v-icon color="blue" class="mr-1">info</v-icon><span v-html="$t('Transaction.Sightline.Funding.Note1')"></span></p>
          <v-btn
            :disabled="isRedirectionInProgress"
            :loading="isRedirectionInProgress"
            color="success"
            @click="submit"
          >{{ $t('Transaction.Sightline.ButtonText1')}}
