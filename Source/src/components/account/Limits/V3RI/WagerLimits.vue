<template>
  <div :class="['page-content account', {'not-acc-sf': !accountSummaryFlow}]">
    <div class="limits" v-if="isLoggedIn">
      ...
      <div class="loader" v-if="!wagerLimits">
        <v-progress-circular indeterminate class="circular-progress"></v-progress-circular>
      </div>
      <div class="limits-container" v-if="wagerLimits">
        <div v-for="(limit, index) in wagerLimits" :key="index">
          <Limit :limit="limit"/>
        </div>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      wagerLimits () {
        return this.playLimits && this.playLimits.filter(l => l.idmmlmtype === 'TRADING')
      }
    },
    mounted () {
