  ...
  <div class="payment__instrument injected" @click="start()">
    <div class="pi-item-wrapper" v-if="!processing">
      ...
      <span class="piitem-text">
        <div class="piitem-text-headline">{{ $t('Payments.PNM.Credit.Headline') }}</div>
        <div class="piitem-text-desc">{{ $t('Payments.PNM.Credit.Desc') }}</div>
      </span>
...
export default {
  ...
  methods: {
    start () {
      if (this.customerCanDeposit) store.dispatch('pnmGateway/pnmCredit/start')
    },
    closePNMIFrame () {
  ...
  computed: {
    processing: () => store.getters['pnmGateway/pnmCredit/processing'],
    defaultProcessingMessage () {
      return this.$t('Transaction.PleaseWait')
    },
    redirectionUrl: () => store.getters['pnmGateway/pnmCredit/redirectionURL'],
