<template>
  <div class="payment__instrument">
    <div v-if="!processing && !fetchingPaymentTypes">
      <div class="label">{{ $t('Payments.PNM.PayPal.Headline') }}</div>
      <div class="desc">{{ $t('Payments.PNM.PayPalWithdrawal.Desc') }}</div>
      <!-- Error Msg -->
      <div v-if="errorMessage" class="pnmpd-failed">{{errorMessage}}</div>
      <!-- Info Msg -->
      <div v-if="infoMessage" class="pnmpd-info"><v-icon color="blue" class="mr-1" size="16">info</v-icon> {{infoMessage}}</div>
      <!--Template to display if there is no paypal acc added -->
      <div class="pnmpd-cards" v-if="!paypalAccount">
        <!-- changed because of MEGA-293-->
        <p class="no__cards__info"><b>{{ $t('Payments.PNM.PayPalWithdrawal.Enable') }}</b></p>
        <div class="no__cards__info">{{ $t('Payments.PNM.PayPalWithdrawal.Info') }}</div>
          <!-- <div class="pnmpd__btn_cont">
          <v-btn large color="success" @click.stop="addPayPalAccount()">{{ $t('Payments.PNM.PushDebit.AddNew') }}</v-btn>
        </div> -->
      </div>
      <div v-else>
        <!--Template to display if Paypal account is added -->
        <!--Form -->
        <div class="pnmpd-cards pnmpd-cards__form" :class="{'pnmpd-cards__form--visible': paypalAccount}">
          ...
          <v-form ref="form" id="form" v-model="valid" lazy-validation @submit.prevent="submit">
            <v-text-field
              ...
              required
              :disabled="processing"
              :rules="[formRules.amount.required, formRules.amount.number]"
...
export default {
  ...
  methods: {
    ...
    addPayPalAccount () {
      try {
        store.dispatch('pnmGateway/pnmPayPal/start', 'addNewAccount')
        this.disposeLocalState()
      } catch (ex) {
        console.log(ex)
      }
  ...
  computed: {
    ...
    processing () {
      return store.getters['pnmGateway/pnmPushDebit/getProcessing'] || store.getters['pnmGateway/pnmPayPal/getProcessing']
    },
    defaultProcessingMessage () {
      return this.$t('Transaction.PleaseWait')
    },
    errorMessage () {
      const msg = store.getters['pnmGateway/pnmPushDebit/errMsg']
      return msg
    },
    paypalAccount: () => {
      let instruments = store.getters['payments/activePaymentInstuments']
      return instruments && instruments
        .find(type => type.IDMMSIType === 'PNMPushDebit' && type.VerificationStatus === -1 && type.ExtraValue1 === 'PayPal')
    },
    defaultInstument () {
      let instruments = store.getters['payments/activePaymentInstuments']
      return instruments && instruments.filter(type => type.IDMMSIType === 'PNMPushDebit')[0]
    },
    redirectionUrl: () => store.getters['pnmGateway/pnmPayPal/redirectionURL'],
    ...
    isPaypalWithdrawal () {
      return this.$route.name.includes('pnm-paypal-withdrawal')
    }
  },
  created () {
    viewModel = this
...
window.addEventListener('message', params => {
  if (params && params.data && params.data.data === 'closePNMPushDebitIFrame') {
    ...
    if (viewModel) {
      if (viewModel.isPaypalWithdrawal) {
