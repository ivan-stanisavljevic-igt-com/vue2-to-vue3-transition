  ...
  <div class="payment__instrument injected" @click="start()">
    <div class="pi-item-wrapper" v-if="!processing">
      ...
      <span class="piitem-text">
        ...
        <div class="piitem-text-desc">{{ $t('Payments.PNM.PayPal.Desc') }}</div>
      </span>
...
export default {
  ...
  methods: {
    start () {
      if (this.customerCanDeposit) store.dispatch('pnmGateway/pnmPayPal/start')
    },
    closePNMIFrame () {
  ...
  computed: {
    processing: () => store.getters['pnmGateway/pnmPayPal/processing'],
