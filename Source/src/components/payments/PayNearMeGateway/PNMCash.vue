<template>
  <div class="payment__instrument injected" @click="start()">
    <div class="pi-item-wrapper" v-if="!processing">
      <span class="piitem-picture"><img :src="mediaServer('/static/brand-img/' + brandKey + '/paynearme.svg')" alt="cash"/></span>
      <span class="piitem-text">
        <div class="piitem-text-headline">{{ $t('Payments.PNM.Cash.Headline') }}</div>
        <div class="piitem-text-desc">{{ $t('Payments.PNM.Cash.Desc') }}</div>
      </span>
...
export default {
  ...
  methods: {
    start () {
      if (this.customerCanDeposit) store.dispatch('pnmGateway/pnmCash/start')
    },
    closePNMIFrame () {
  ...
  computed: {
    processing: () => store.getters['pnmGateway/pnmCash/processing'],
    defaultProcessingMessage () {
      return this.$t('Transaction.PleaseWait')
    },
    redirectionUrl: () => store.getters['pnmGateway/pnmCash/redirectionURL'],
