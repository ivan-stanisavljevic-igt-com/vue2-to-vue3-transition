<template>
  <div class="payment__instrument injected" @click="start()">
    <div class="pi-item-wrapper" v-if="!processing">
      <span class="piitem-picture"><img :src="mediaServer('/static/brand-img/' + brandKey + '/ach.svg')" alt="ach"/></span>
      <span class="piitem-text">
        <div class="piitem-text-headline">{{ $t('Payments.PNM.Ach.Headline') }}</div>
        <div class="piitem-text-desc">{{ $t('Payments.PNM.Ach.Desc') }}</div>
      </span>
...
export default {
  ...
  methods: {
    start () {
      if (this.customerCanDeposit) store.dispatch('pnmGateway/pnmAch/start')
    },
    closePNMIFrame () {
  ...
  computed: {
    processing: () => store.getters['pnmGateway/pnmAch/processing'],
    defaultProcessingMessage () {
      return this.$t('Transaction.PleaseWait')
    },
    redirectionUrl: () => store.getters['pnmGateway/pnmAch/redirectionURL'],
