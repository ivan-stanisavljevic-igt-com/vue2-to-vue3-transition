<template>
  <div class="payment__instrument injected" @click="start()">
    <div class="pi-item-wrapper" v-if="!processing">
      <span class="piitem-picture"><img :src="mediaServer('/static/brand-img/' + brandKey + '/venmo.svg')" alt="venmo"/></span>
      <span class="piitem-text">
        <div class="piitem-text-headline">{{ $t('Payments.PNM.Venmo.Headline') }}</div>
        <div class="piitem-text-desc">{{ $t('Payments.PNM.Venmo.Desc') }}</div>
      </span>
...
export default {
  ...
  methods: {
    start () {
      if (this.customerCanDeposit) store.dispatch('pnmGateway/pnmVenmo/start')
    },
    closePNMIFrame () {
  ...
  computed: {
    processing: () => store.getters['pnmGateway/pnmVenmo/processing'],
    defaultProcessingMessage () {
      return this.$t('Transaction.PleaseWait')
    },
    redirectionUrl: () => store.getters['pnmGateway/pnmVenmo/redirectionURL'],
