<template>
  <div class="payment__instrument injected" @click="start()">
    <div class="pi-item-wrapper">
      <span class="piitem-picture"><img :src="mediaServer('/static/brand-img/' + brandKey + '/paynearme.svg')" alt="nuvei-pnm"/></span>
      <span class="piitem-text">
        <div class="piitem-text-headline">{{ $t('Payments.PNM.Cash.Headline') }}</div>
        <div class="piitem-text-desc">{{ $t('Payments.Nuvei.PNM.Desc') }}</div>
      </span>
