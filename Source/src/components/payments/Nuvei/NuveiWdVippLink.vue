<template>
  ...
  >
    <span class="piitem-picture"><img :src="mediaServer('/static/brand-img/' + brandKey + '/vip-preferred.svg')" alt="vipp"/></span>
    <span class="piitem-text">
      <div class="piitem-text-headline">
        {{ $t('Payments.Nuvei.VIPP') }}
        <span v-if="!hasMadeDeposit && customerCanWithdraw" class="piitem-must-deposit">{{ $t('Payments.PNM.PayPalWithdrawal.Enable') }}</span>
      </div>
       <div class="piitem-text-desc">{{ $t('Payments.Nuvei.VIPP.Desc.Wd') }}</div>
    </span>
