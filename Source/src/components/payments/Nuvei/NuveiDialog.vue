<template>
  <v-dialog
    ...
    <v-card class="pnmgw__dialog__card" :class="{ 'hidden': iFrameIsLoading }">
      <NuveiDialogHeader :type="type" @closeNuveiDialog="closeAction()"/>
...
import NuveiDialogHeader from '@/components/payments/nuvei/NuveiDialogHeader'
...
export default {
  ...
  computed: {
    show: {
      ...
      set () {
        this.dialog = false
        store.commit(`nuvei/${this.type}/redirectionURL`, undefined)
      }
    }
  },
  methods: {
    closeDialog () {
      ...
      this.dialog = false
      store.commit(`nuvei/${this.type}/redirectionURL`, undefined)
      store.commit(`nuvei/setCurrentRef`, null)
    },
    stopSpinner () {
  ...
  mounted () {
    store.commit(`nuvei/setCurrentRef`, this.type)
