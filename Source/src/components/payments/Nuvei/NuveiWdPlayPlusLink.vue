<template>
  ...
  >
    <span class="piitem-picture"><img :src="mediaServer('/static/brand-img/' + brandKey + '/sightline.png')" alt="sightline"/></span>
    <span class="piitem-text">
      <div class="piitem-text-headline">
        {{ $t('Transaction.Sightline') }}
        <span v-if="!hasMadeDeposit && customerCanWithdraw" class="piitem-must-deposit">{{ $t('Payments.PNM.PayPalWithdrawal.Enable') }}</span>
      </div>
      <div class="piitem-text-desc">{{ $t('Payments.Nuvei.PlayPlus.Wd.Desc') }}</div>
    </span>
